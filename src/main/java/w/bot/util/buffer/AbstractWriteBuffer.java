...
public abstract class AbstractWriteBuffer implements WriteBuffer {
    ...
    protected AbstractWriteBuffer(final byte[] array) {
        this.array = array;
    }
    ...
    @Override
    public @NotNull WriteBuffer write(final @NotNull String text) {
    ...
    @Override
    public @NotNull WriteBuffer write(final byte @NotNull [] bytes) {
        return write(bytes, 0, bytes.length);
    }

    @Override
    public @NotNull WriteBuffer write(final byte @NotNull [] bytes, final int off, final int len) {
        ensure(len);
